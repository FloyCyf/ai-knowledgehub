...
> 如果你的环境安装的是旧版 Docker Compose，也可以使用 `docker-compose up -d`。新版 Docker Desktop 推荐使用 `docker compose`。
...
Windows 本地也可以使用辅助脚本按同样顺序启动并记录日志：
...
# 文章详情限流默认 10 秒 20 次，第 21 次返回 429
for i in {1..25}; do
...
A：用户接口路径统一为 `/api/user/**`。请使用 `http://localhost:8080/api/user/login`。网关不会剥离 `/api` 前缀，下游服务也保留 `/api/user` 映射。
...
| ai-service | ✅ 已完成 | C | AI 续写、SSE、MQ 消费、标签提取、合规检测 |
